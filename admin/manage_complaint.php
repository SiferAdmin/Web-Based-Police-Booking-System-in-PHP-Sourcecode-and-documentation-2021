...
<?php
	...
	if($status > 1){
		$aqry =  $conn->query("SELECT * FROM bookingaction where complaint_id = {$_GET['id']} ");
			...
			<div class="assign-responder" style="display:none">
				<div class="form-group">
					...
					<select name="responder_id" id="" class="custom-select input-sm select2">
						<option value=""></option>
						<?php
							$booking = $conn->query("SELECT rt.*,s.name as sname,s.address FROM responders_team rt inner join stations s on s.id = rt.station_id where rt.availability = 1 ".(isset($ca['responder_id']) ? " or rt.id = {$ca['responder_id']}" : '')." order by rt.name asc ");
